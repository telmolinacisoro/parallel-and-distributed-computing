...
//Prints all the numbers in the grid
...
//Check if a number already exists in a box
...
//Call the three previous functions to check if a number can be placed in one position
...
int solve(int grid[SIZE][SIZE], int level) {
    ...
    //This returns 1 when there are no more values to assign
    if (!find_unassigned(grid, &row, &col)) return 1;

        //This loop adds a value to the next unassigned number
        for (int num = 1; num <= SIZE; num++ ) {
            if (is_safe_num(grid, row, col, num)) {
         
                //Create the tasks
                #pragma omp task firstprivate(grid,row,col,level,num) final(level>1)
                {
