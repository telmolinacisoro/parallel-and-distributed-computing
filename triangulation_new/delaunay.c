        ...
        for (int j = i + 1; j < num_points; j++) {
            ...
            for (int k = j + 1; k < num_points; k++) {
                ...
                for (int m = 0; m < num_points; m++) {
                    if (m != i && m != j && m != k && inside_circle(&points[m], &(struct Triangle){*p1,*p2,*p3})){
                        inside = FALSE;
                        break;
                        }
                    }
            ...
            if (image[pixel_index] == -1){
                for (int k = 0; k < num_triangles; k++) { 
                    ...
                    if (inside_triangle(triangle, &(struct Point) {(double)i, (double)j})) {
                        ...
                        image[pixel_index] = alpha * triangle->p1.value + beta * triangle->p2.value + gamma * triangle->p3.value;
                        break;
